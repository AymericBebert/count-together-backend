...
docker build -t aymericbernard/count-together-backend:$version --build-arg BUILD_CONFIGURATION=$BUILD_CONFIGURATION --build-arg VERSION=$version . || { echo 'build failed' ; exit 1; }

echo "Pushing count-together-backend:$version to docker registry..."
docker push aymericbernard/count-together-backend:$version || { echo 'push failed' ; exit 1; }
